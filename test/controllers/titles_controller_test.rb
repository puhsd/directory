...
class TitlesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create title" do
    assert_difference('Title.count') do
      post titles_url, params: { title: { name: @title.name, public: @title.public } }
    end

    assert_redirected_to title_path(Title.last)
  end
  ...
  test "should update title" do
    patch title_url(@title), params: { title: { name: @title.name, public: @title.public } }
    assert_redirected_to title_path(@title)
  end
